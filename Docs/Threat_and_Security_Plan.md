...
| 5 | A08: Data Integrity | `pickle` import and `pickle.load()` deserialisation risk (Bandit B403/B301). If an attacker can replace `.pkl` files (path traversal, compromised deployment), they achieve Remote Code Execution. Hash verification has been implemented as a mitigation. | `pomodoro/ml/trainer.py:45-58,97-100` | Critical | **Hash verification before deserialization** — the model's SHA-256 hash is computed at training time and stored inside `meta.pkl` as `model_hash`. `load_model()` (line 45) recomputes the hash of `model.pkl` on disk and compares it against the stored hash before `pickle.load()`, raising `ValueError` on mismatch. Legacy models without the hash field are accepted gracefully. |
